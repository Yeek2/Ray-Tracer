...
class vec3{
    ...
    public:
        ...
        bool    operator!=(const vec3& ve)  const { return !operator==(ve); }                                                                                   //Overload "!=" operator to compare element-wise each element of the array by calling the equals function, and negating it
        ...
        double& operator[](int index) { if (index < 0 || index >= 4) { throw std::out_of_range("Index out of bounds for vec3"); } return v[index]; }            //Overload "[]" operator to get the index of the array
        const double& operator[](int index) const { if (index < 0 || index >= 4) { throw std::out_of_range("Index out of bounds for vec3"); } return v[index]; }//Overload "[]" operator to get the index of the array for constants
